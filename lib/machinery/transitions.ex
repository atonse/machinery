defmodule Machinery.Transitions do
  ...
  def handle_call({:run, struct, state_machine_module, next_state, metadata}, _from, states) do
    ...
    guarded_transition? = Transition.guarded_transition?(state_machine_module, struct, next_state, metadata)
    ...
    response = cond do
      ...
      true ->
        struct = struct
          |> Transition.before_callbacks(next_state, state_machine_module, metadata)
          |> Transition.persist_struct(next_state, state_machine_module, metadata)
          |> Transition.log_transition(next_state, state_machine_module, metadata)
          |> Transition.after_callbacks(next_state, state_machine_module, metadata)
        {:ok, struct}
    end
